    ...
    def on_thumbnail_downloaded(self, video_id: str, url: str, reply: QNetworkReply):
        if reply.error() == QNetworkReply.NetworkError.NoError:
            ...
            if not pixmap.isNull():
                scaled_pixmap = pixmap.scaled(120, 68, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
        ...
        try:
            ...
            try:
                if hasattr(self.worker, 'error_occurred'):
                    if hasattr(self.worker, 'error_occurred'):
                        getattr(self.worker, 'error_occurred').emit(str(e))
            except Exception:
                pass
...
class VideoItemWidget(QWidget):
    def __init__(self, video_data: dict, thumbnail_loader: ThumbnailLoader):
        ...
        # Quality selection (will be updated based on global format)
        self.quality_label = QLabel("Quality:")
        layout.addWidget(self.quality_label)
        self.quality_combo = QComboBox()
        self.quality_combo.addItems(video_data.get('qualities', ['Best Available']))
    ...
    def update_format(self, format_type: str):
        """Update quality options based on global format selection"""
        if format_type == 'mp3':
            # Update to MP3 qualities
            self.quality_combo.clear()
            self.quality_combo.addItems(['320 kbps', '192 kbps', '128 kbps', 'Best Audio'])
        elif format_type == 'mp4':
            # Update to MP4 qualities
            self.quality_combo.clear()
            qualities = self.video_data.get('qualities', ['Best Available'])
            self.quality_combo.addItems(qualities)
        elif format_type == 'image':
            # For images, we don't need quality selection
            self.quality_combo.clear()
            self.quality_combo.addItems(['Original'])
...
class DownloadWorker(QObject):
    ...
    def __init__(self, item_id: str, video_data: dict, output_path: str, quality: str, subtitle: str, audio_only: bool, fetch_images: bool = False, fetch_all: bool = False):
        super().__init__()
        self.item_id = item_id
        self.video_data = video_data
...
class YouTubeDownloaderGUI(QWidget):
    ...
    def _on_format_changed(self, button):
        """Update all video items when format is changed"""
        if self.mp3_radio.isChecked():
            ...
            self.selected_format = 'image'
        
        # Update all video items
        for video_item in self.video_items:
            if hasattr(video_item, 'update_format'):
                video_item.update_format(self.selected_format)
    ...
    def init_ui(self):
        ...
        # Add scroll area to this widget
        ...
        # Add format selection radio buttons (affects all videos)
        self.format_group = QButtonGroup()
        self.mp4_radio = QRadioButton("🎥 MP4 Video")
        self.mp3_radio = QRadioButton("🎵 MP3 Audio")
        self.image_radio = QRadioButton("🖼️ Images")
        ...
        format_layout.addWidget(QLabel("Download All as:"))
        ...
        # Connect radio buttons to update all video items
    ...
    def browse_output_directory(self):
        """Browse for output directory with Desktop as default if none selected"""
        try:
            ...
            # If user cancels or selects nothing, use Desktop
            if not directory:
                desktop_path = str(Path.home() / "Desktop")
                directory = desktop_path
                self.log_message(f"📁 No directory selected. Using Desktop: {directory}")
            else:
    ...
    def toggle_select_all(self, state):
        for item_widget in self.video_items:
            item_widget.checkbox.setChecked(state == Qt.CheckState.Checked)
    ...
    def start_download(self):
        ...
        for i, item_widget in enumerate(self.video_items):
            if item_widget.is_selected():
                ...
                video_data['selected_format'] = self.selected_format  # Use global selected format
    ...
    def start_next_downloads(self):
        ...
        while (self.current_downloads < self.max_concurrent_downloads and 
            ...
            selected_format = video.get('selected_format', 'mp4')
            audio_only = selected_format == "mp3"
            fetch_all = False
            fetch_images = selected_format == "image"
            
            # For YouTube, when image format is selected, we only want thumbnails
            if fetch_images and 'youtube.com' in video.get('url', ''):
                fetch_images = True
                audio_only = False
                # We'll handle thumbnail download specially in the download function
